(()=>{
  'use strict';
  const BUILD='2026-07-04-route-1';
  ...
  function renderCard(row,request){
    ...
    if(!row){
      ...
      card.innerHTML=`<p class="eyebrow">MIMI 21 ROUTE ERROR</p><h2>Unknown fish</h2><p>${request.fish||request.slot}</p><small>Check the NFC URL against data/nfc-tags-21-v2.csv</small>`;
      return;
    }
    card.className='mimi21-route-card';
    card.innerHTML=`<p class="eyebrow">${row.slot} · ${row.category}</p><h2>${row.name}</h2><code>${row.code}</code><p>${row.write_status==='pilot'?'Pilot tag':'Birthday V2 tag'}</p><small>Seed ${2100+slotNumber(row.slot)} · ${row.id}</small>`;
  ...
  async function applyRoute(){
    ...
    if(!request)return;
    if(request.set&&request.set!==TARGET_SET)return;
  ...
  function run(){
    ...
    applyRoute().catch(error=>{
      ...
      const request=routeRequest();
      if(request)renderCard(null,request);
    });
    if(tries<5)setTimeout(run,tries*280);
  }
